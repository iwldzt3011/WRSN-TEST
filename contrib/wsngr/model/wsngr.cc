...
void NodeInfo::handleEnergy(int type,double Consumption_coefficient){
  ...
  if(energy <= 0){
    if(state != NodeState::DEAD){
      ...
      energy = 0;
      if(first_dead_time==Seconds(7200)){
         first_dead_time=Simulator::Now();
      }
    }
  }
}
                              ...
                              Socket::SocketErrno &sockerr)
{
  ...
  double transmission_distance=CalculateDistance(nodes[next].position,nodes[m_mainAddress].position);
  double Consumption_coefficient=std::max(transmission_distance/COMM_RANGE,1.0);
  ...
  GetInfo().handleEnergy(NodeInfo::TX_TYPE,Consumption_coefficient);
                             ...
                             ErrorCallback ecb)
{
  ...
  GetInfo().handleEnergy(NodeInfo::RX_TYPE,1);
  ...
  Ipv4Address next = GetNextForward ();
  double transmission_distance=CalculateDistance(nodes[next].position,nodes[m_mainAddress].position);
  double Consumption_coefficient=std::min(transmission_distance/COMM_RANGE,1.0);
...
void RoutingProtocol::SensingConsume(){
  GetInfo().handleEnergy(NodeInfo::SENSING_TYPE,1);
